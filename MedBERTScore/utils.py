...
def sent_encode(tokenizer, sent, sliding_window_flag=False):
        ...
        if version.parse(trans_version) >= version.parse("4.0.0"):
            if sliding_window_flag:
                ...
                return tokenizer.encode(
                    ...
                    max_length=  tokenizer.model_max_length,
                    truncation= True,
                    #comment max_lenght and truncation for sliding window 
...
def get_model(model_type, num_layers, all_layers=None):
    ...
    elif "bigbird" in model_type:
        from transformers import BigBirdModel
        model = BigBirdModel.from_pretrained(model_type)
    else:
...
def get_tokenizer(model_type, use_fast=False):
    ...
    if version.parse(trans_version) >= version.parse("4.0.0"):
        tokenizer = AutoTokenizer.from_pretrained(model_type, use_fast=use_fast, add_prefix_space=True)
    else:
...
def bert_encode(model, x, attention_mask, all_layers=False, pad=1, sliding_window_flag=False):
    model.eval()
    ...
    if sliding_window_flag:
        emb= sliding_window(model, x, attention_mask,512, all_layers, pad)
    else:
        emb = base_embedding(model, x, attention_mask, all_layers, pad)

    return emb

def base_embedding(model, x, attention_mask, all_layers=False, pad=1):
    with torch.no_grad():
...
def sliding_window(model, x, attention_mask,model_max_lenght=512, all_layers=False,pad=1, dtype=torch.long):
    final_emb = None
    previous_window = 100
        ...
        if tensor_temp_size > model_max_lenght: 
            ...
            while index*window_lenght < tensor_temp_size:  
                if index == 0:
                    ...
                    input_emb = input[index*window_lenght :(index+1)*window_lenght].unsqueeze(0)
                    att_emb = attention[index*window_lenght :(index+1)*window_lenght].unsqueeze(0)
                else:
                    ...
                    if input.size()[0] -previous_window == input_emb.size()[1]:
                        input_emb = torch.cat([input_emb, input[previous_window:].unsqueeze(0)], dim=0 )
                        att_emb = torch.cat([att_emb, attention[previous_window:].unsqueeze(0)], dim=0 )
                    else:
                        padded = torch.ones(1, input_emb.size()[1], dtype=dtype)  * pad
                        mask = torch.zeros(1, input_emb.size()[1], dtype=torch.long)
                        padded[0, : input.size()[0]-previous_window] = input[previous_window:]
                        mask[0, : input.size()[0]-previous_window] = 1
                        input_emb = torch.cat([input_emb, padded.to(input_emb.device)], dim=0 )
                        att_emb = torch.cat([att_emb, mask.to(input_emb.device)], dim=0 )

                index = index + 1
            with torch.no_grad():
                out = model(input_emb, att_emb, output_hidden_states=all_layers)
            for  j in range(0, out[0].size()[0]):
                if emb is None:
                    emb = out[0][j].unsqueeze(0)
                else:
                    emb = torch.cat([emb, out[0][j].unsqueeze(0)], dim=1 )
            emb = emb[:,:tensor_temp_size, :]
...
def get_idf_dict(arr, tokenizer, nthreads=4,sliding_window_flag=False):
    """
    ...
    process_partial = partial(process, tokenizer=tokenizer, sliding_window_flag=sliding_window_flag)
...
def data_iterator_test(token_list):
...
def collate_idf(arr, tokenizer, idf_dict, idf_per_text=None, cat=None, id_text_flag=True, device="cuda:0", sliding_window_flag=False):
    """
    ...
    arr_temp = arr
    arr = [sent_encode(tokenizer, a,sliding_window_flag=sliding_window_flag ) for a in arr]
    idf_weights = []
    
    if idf_per_text is not None:
        idf_results = idf_per_text
        for doc_id in range(0, len(arr)):
            temp = idf_results[arr_temp[doc_id]]
            
            temp[0] = 0
            temp = temp[:len(arr[doc_id])-1]
            temp.append(0)
            idf_weights.append(temp)
    else: 
        idf_results = idf_dict
...
def get_bert_embedding(
    ...
    batch_size=-1,
    device="cuda:0",
    all_layers=False, sliding_window_flag=False
):
    """
    ...
    padded_sens, padded_idf, lens, mask = collate_idf(
        all_sens, tokenizer, idf_dict, idf_per_text, cat, device=device, sliding_window_flag=sliding_window_flag
    )
    padded_sens.to(device)
    padded_idf.to(device)
    mask.to(device)
    ...
    with torch.no_grad():
        for i in range(0, len(all_sens), batch_size):
            batch_embedding = bert_encode(
                ...
                pad = tokenizer.pad_token_id, sliding_window_flag=sliding_window_flag
            )
...
def greedy_cos_idf(
    ...
    hyp_temp = 1.0/hyp_idf.sum(dim=1)
    ref_temp = 1.0/ref_idf.sum(dim=1)
    ...
    cosine_cls = torch.nn.CosineSimilarity()
...
def bert_cos_score_idf(
    ...
    stats_dict = dict()
    for batch_start in tqdm(iter_range): #by sorting and dedup we calculate each unique sentenece once
        sen_batch = sentences[batch_start : batch_start + batch_size]
        embs, masks, padded_idf = get_bert_embedding(
            sen_batch, model, tokenizer, idf_dict, idf_per_text,cat, device=device, all_layers=all_layers, sliding_window_flag=sliding_window_flag
        )
    ...
    def pad_batch_stats(sen_batch, stats_dict, device):
        ...
        emb, idf, cls_token= zip(*stats)
        cls_token = [cls_t.to(device) for cls_t in cls_token]
