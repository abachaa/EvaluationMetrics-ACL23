...
class Scorer:
    ...
    def __init__(self, file_path, device='cuda:0', multi_ref=False, weight=False, bart_path=None):
        """ file_path: path to the pickle file
            All the data are normal capitalized, and tokenized, including src, ref_summ, ref_summs, and sys_summ.
        """
        self.max_length = 1024
        self.weight_flag = weight
        ...
        if not multi_ref:
            ...
            self.single_ref_weights = None
            if self.weight_flag:
                self.single_ref_weights = self.get_single_ref_weight()
    ...
    def get_single_ref_weight(self):
        ref_weight = []
        for doc_id in self.data:
            temp = self.data[doc_id]['ref_weight'][:self.max_length-1]
            temp.append(1)
            weight_tensor = torch.FloatTensor(temp)
            if weight_tensor.size()[0] < self.max_length:
                weight_tensor = F.pad(input=weight_tensor, pad=(0, self.max_length - weight_tensor.size()[0]),
                                      mode='constant', value=1)
 
            ref_weight.append(weight_tensor)
    ...
    def get_sys_weight(self, sys_name):
        sys_weight = []
        for doc_id in self.data:
            temp = self.data[doc_id]['sys_summs'][sys_name]['sys_weight'][:self.max_length-1]
            temp.append(1)
            ...
            sys_weight.append(weight_tensor)
        return sys_weight
    ...
    def get_src_weights(self):
        src_weights = []
        for doc_id in self.data:
            temp = self.data[doc_id]['src_weight'][:self.max_length-2]
            temp.insert(0, 1)
            temp.append(1)
            weight_tensor = torch.FloatTensor(temp)
            ...
            src_weights.append(weight_tensor)
        return src_weights
        ...
        for metric_name in metrics:
            if metric_name == 'bert_score':
                ...
                if 'cnn' in metric_name:
                    ...
                    bart_scorer = BARTScorer(device=self.device, checkpoint='facebook/bart-large-cnn')
                    bart_scorer.load(self.bart_path)
                else:
                ...
                if not self.multi_ref:
                    ref_lines = [detokenize(line) for line in self.single_ref_lines]
                    ref_weight = self.single_ref_weights
                else:
                    ref_lines = [[detokenize(text) for text in line] for line in self.multi_ref_lines]
                for sys_name in self.sys_names:
                    sys_lines = self.get_sys_lines(sys_name)
                    sys_weight = None
                    if self.weight_flag:
                        sys_weight = self.get_sys_weight(sys_name)
                    
                    sys_lines = [detokenize(line) for line in sys_lines]
                    
                   
                    src_hypo = bart_scorer.score(src_lines, sys_lines, src_weights, sys_weight, batch_size=4, promtp=3)
            
                    if not self.multi_ref:
                        ref_hypo = np.array(bart_scorer.score(ref_lines, sys_lines, ref_weight, sys_weight, batch_size=4,promtp=3))
                        hypo_ref = np.array(bart_scorer.score(sys_lines, ref_lines, sys_weight, ref_weight, batch_size=4, promtp=3))
                    else:
...
def main():
    ...
    parser.add_argument('--weight', action='store_true', default=False,
                        help='Whether to calculate a weighted Bartscore version')
    ...
    scorer = Scorer(args.file, args.device, args.multi_ref, args.weight, args.bart_path)
    ...
    print("running time :", (end-start) , "second")
