...
class Medical_Weight:
    ...
    def __init__(self, file_path, device='cuda:0',  max_length=1024,  checkpoint='facebook/bart-large'):
        ...
        self.tokenizer = BartTokenizerFast.from_pretrained(checkpoint, return_offsets_mapping=True)
        print("done! setup")
    ...
    def medical_tokenizer(self,  srcs,   batch_size=1):
        ...
        final_encoding = {}
        srcs = [detokenize(line) for line in srcs]
                ...
                if len(annotation['type_ids']) > 0:  
                    if annotation['type_ids'][0] in tui_id:                      
                        ...
                        if start not in entity_id:
                            entity_id[start] = end_entity 
                        else:
                            if end_entity >entity_id[start]:
                                entity_id[start]= end_entity 
    ...
    def get_weight(self, candidate_path, reference_path, output_path):
        ...
        output = open(output_path + "weight_ref.pkl", "wb")
        ...
        # Run model
        results = self.cat.multiprocessing(self.data_iterator_test(token_list), batch_size_chars=batch_size_chars, nproc=4)  
        weigths = self.create_bartweight(token_list, results)
        output = open(output_path + "weight_cands.pkl", "wb")
        pickle.dump( weigths, output)
...
parser.add_argument('--file', type=str,    help='The MedCat file path')
...
output = open(args.output + "weight_time.txt", "w")
